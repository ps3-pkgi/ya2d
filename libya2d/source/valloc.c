...
static u64 VRAM_SIZE = 0;
static u64 VRAM_BASE = 0;
...
vram_mem_header_t * _vram_mem_fit(vram_mem_header_t *head, size_t size)
{
	...
	u64 prev_top, next_bot;
	size_t best_size = 0;

	if (((u64)head->ptr+head->size+size)<VRAM_SIZE) {
		best_fit = head;
		best_size = VRAM_SIZE - ((u64)head->ptr+head->size);
	}

	while (prev_mem != NULL) {
		if (prev_mem->next != NULL) {
			prev_top = (u64)prev_mem->ptr;
			next_bot = prev_top - ((u64)prev_mem->next->ptr + prev_mem->next->size);
			if (next_bot >= size) {
...
void * valloc(size_t size)
{
	...
	/*if (((u64)__valloc_vram_head->ptr + __valloc_vram_head->size + mem_sz) < VRAM_SIZE) {
		new_mem = (vram_mem_header_t *)malloc( sizeof(vram_mem_header_t) );
		if (new_mem == NULL)
			return ptr;
		ptr     = (void *)((u64)__valloc_vram_head->ptr + __valloc_vram_head->size);
	...
	if (prev_mem != NULL) {
		...
		if (new_mem == NULL)
			return ptr;
		ptr     = (void *)((u64)prev_mem->ptr + prev_mem->size);
...
size_t vmemavail()
{
	...
	vram_mem_header_t *cur;
	size_t size = VRAM_SIZE - ((u64)__valloc_vram_head->ptr + __valloc_vram_head->size);

	cur = __valloc_vram_head;
	while (cur->next!=NULL)  {
		size += (u64)cur->ptr - ((u64)cur->next->ptr + cur->next->size);
		cur = cur->next;
	}
...
size_t vlargestblock()
{
	...
	cur = __valloc_vram_head;
	while (cur->next!=NULL)  {
		new_size = (u64)cur->ptr - ((u64)cur->next->ptr + cur->next->size);
